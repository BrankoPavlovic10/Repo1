	...
	<body>
		...
		<div class="page">
			<div class="column">
				...
				<div class="content" id="uloguj-se">
					<div class="petition">
						<div class="login">
							<h1>Stranica za logovanje</h1>
							
							<?php
							
							if( isset($_SESSION['mejl'] ) ){
							 echo '<div style="text-align:center;font-size:40px; ">  Uspesno ste registrovani! 
								    <form action="php/logout.php" method="post">
								 <button type="submit" name="dugme-logout">Izloguj se</button>
								 </form> </div>'  ;
							}else{
								if(isset($_GET['error'])  ){//provera koja sledi nakon sto se vratimo na ovu stranicu
	                                if($_GET['error'] == "losMejl" ){
										echo '<p style="text-align:center;font-size:30px">Unet je neodgovarajuc mejl. </p>';
	                                    }elseif($_GET['error'] == "losaSifra"  ) {
											echo '<p style="text-align:center;font-size:30px;">Uneta je neodgovarajuca sifra';
										}
									
									}
				
						      	echo '<form class="kontakt-forma" method="POST" action="php/login.php">
							       	Na predstojećem formularu se možete ulogovati na naš sajt: <br><br>
								
								    <label for="input-email">E-mail adresa:</label><br>
								    <input type="email" placeholder="Unesite email" name="login-mejl"  required><br><br>
								
								    <label for="input-sifra">Šifra:</label><br>
								    <input type="password" name="login-sifra" placeholder="Unesite sifru" required > <br>
								
								    <button type="submit" name="dugme-logovanje">Uloguj se</button>	
